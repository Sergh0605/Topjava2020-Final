...
> - Правки кода (замечания codacy) и роли в `Topjava-soapui-project`

#### Apply 8_0_2_rename.patch
> - В `TestMatcher` переименовал статический фабричный метод и метод `setUp` в тестах
...
>  - Для `spring-security` взял последнюю версию и на всякий случай обновил остальные версии Spring. **Не забудьте отдать `mvn clean`**
...
- 1: Перевести `meals` на `datatables` (`meals.jsp`, `MealUIController`).
